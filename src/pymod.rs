...
static mut MODULE_DEF: PyModuleDef = PyModuleDef {
    ...
    m_name: cstr!("string_sum"),
    m_doc: cstr!("A Python module written in Rust."),
    m_size: 0,
...
static mut METHODS: &[PyMethodDef] = &[
    PyMethodDef {
        ml_name: cstr!("sum_as_string"),
        ml_meth: PyMethodDefPointer {
            _PyCFunctionFast: sum_as_string,
        },
        ml_flags: METH_FASTCALL,
        ml_doc: cstr!("returns the sum of two integers as a string"),
    },
...
pub unsafe extern "C" fn sum_as_string(
    ...
    if nargs != 2 {
        PyErr_SetString(
            PyExc_TypeError,
            cstr!("sum_as_string expected 2 positional arguments"),
        );
        return std::ptr::null_mut();
    }
    ...
    match first.checked_add(second) {
        ...
        None => {
            PyErr_SetString(PyExc_OverflowError, cstr!("arguments too large to add"));
            std::ptr::null_mut()
        }
    }
}
